...
class TestCOExcitation:
    ...
    def test_co_fit_two_component(self):
        """2-component CO fit on the Joblin et al. NGC 7023 data.

        The Joblin data is well-described by a single temperature, so the
        two-component fit converges to nearly-equal temperatures. This test
        pins the current numerical behavior so the refactor cannot change it.
        """
        intensity = {}
        unit = u.Unit("W m-2 sr-1")
        intensity["COv0-0J4-3"] = Measurement(
        ...
        h = COExcitationFit(list(intensity.values()))
        h.run(components=2)
        assert h.tcold.data == pytest.approx(116.59034765, rel=1e-3)
        assert h.thot.data == pytest.approx(116.59248581, rel=1e-3)
        assert h.cold_colden.data == pytest.approx(3.26323458e16, rel=1e-3)
        assert h.hot_colden.data == pytest.approx(1.04697353e17, rel=1e-3)
