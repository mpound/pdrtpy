...
class TestExcitation:
    ...
    def test_fit(self):
        ...
        Jerrs_pdrtpy_demo = {
            ...
            4: 1.00e-05,
            5: 1.00e-05,
        }  # choose something that will not have equal fractional error, equal error is simplest
        ...
        rotDiag_y = (
            4
            * np.pi
            * np.array([f for f in Jfluxes_pdrtpy_demo.values()])
            / scale
            * (np.array([wav_H2[Jl] / Aul_H2[Jl] / gu_H2[Jl + 2] for Jl in Jfluxes_pdrtpy_demo.keys()]))
        )
        rotDiag_e = (
            4
            * np.pi
            * np.array([f for f in Jerrs_pdrtpy_demo.values()])
            / scale
            * (np.array([wav_H2[Jl] / Aul_H2[Jl] / gu_H2[Jl + 2] for Jl in Jerrs_pdrtpy_demo.keys()]))
        )
