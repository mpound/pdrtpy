...
class TestExcitation:
    ...
    def test_fit(self):
        ...
        rotDiag_y = 4*np.pi * np.array([f for f in Jfluxes_pdrtpy_demo.values()]) / scale * (np.array([wav_H2[Jl]/Aul_H2[Jl]/gu_H2[Jl+2] for Jl in Jfluxes_pdrtpy_demo.keys()]))
        rotDiag_e = 4*np.pi * np.array([f for f in Jerrs_pdrtpy_demo.values()]) / scale * (np.array([wav_H2[Jl]/Aul_H2[Jl]/gu_H2[Jl+2] for Jl in Jerrs_pdrtpy_demo.keys()]))
        sigma = rotDiag_e/rotDiag_y
        fit, err = curve_fit(self.func2T, rotDiag_x, np.log(rotDiag_y), p0=(1/700,20,1/200,21), bounds=(0,np.inf), max_nfev=8192, absolute_sigma=False, sigma=sigma)
        T1fit = 1/fit[0]
        T1fit_err = np.sqrt(err[0,0])/fit[0]**2
        T2fit = 1/fit[2]
        T2fit_err = np.sqrt(err[2,2])/fit[2]**2
        #print("\n Results of own fit:")
